    ...
    for frame_idx, (path, img, im0s, vid_cap) in enumerate(dataset): #for every frame
        ...
        for i, det in enumerate(pred): #for each detection in this frame
            ...
            if save_txt and len(tracked_dets) != 0:
                for j, tracked_dets in enumerate(tracked_dets):
                    ...
                    with open(txt_path, 'a') as f:
                        ...
                        f.write(f'{frame_idx},{x},{y},{identity},{count}\n')
            ...
            for track in tracks:
                  ...
                  label0 = f'The predicted count: {count}'
                  cv2.putText(im0, label0, org=(100, 100), fontFace=cv2.FONT_HERSHEY_PLAIN, fontScale=5,
                                        color=(255, 255, 0), thickness=5)
                  label1 = f'flying_in : {flying_in}'
                  label2 = f'flying_out : {flying_out}'
                  label3 = f'crawling : {crawling}'
                  
                  cv2.putText(im0, label1, org=(100, 150), fontFace=cv2.FONT_HERSHEY_PLAIN, fontScale=3,
                                  color=(0, 255, 0), thickness=3)
                  cv2.putText(im0, label2, org=(100, 200), fontFace=cv2.FONT_HERSHEY_PLAIN, fontScale=3,
                                  color=(255, 0, 0), thickness=3)
                  cv2.putText(im0, label3, org=(100, 250), fontFace=cv2.FONT_HERSHEY_PLAIN, fontScale=3,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str,
                        default='/content/gdrive/MyDrive/classy-sort-yolov5/yolov5/weights/1.pt', help='model.pt path')
